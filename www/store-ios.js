...
(function () {
    ...
    store.Product = function ( options ) {
        ...
        ///  - `product.state` - Current state the product is in (see [life-cycle](#life-cycle) below). Should be one
        // of the defined [product states](#product-states)
    ...
    /// #### Notes
    ///
    ///  - When finished, a consumable product will get back to the `VALID` state, while other will enter the `OWNED`
    // state.
    ///  - Any error in the purchase process will bring a product back to the `VALID` state.
    ///  - During application startup, products may go instantly from `REGISTERED` to `APPROVED` or `OWNED`, for
    // example if they are purchased non-consumables or non-expired subscriptions. /  - Non-Renewing Subscriptions are
    // iOS products only. Please see the [iOS Non Renewing Subscriptions documentation](https:
    // //github.com/j3k0/cordova-plugin-purchase/blob/master/doc/ios.md#non-renewing) for a detailed explanation. / /
    // #### state changes / / Each time the product changes state, appropriate events is triggered. / / Learn more
    // about events [here](#events) and about listening to events [here](#when). /
...
(function () {
    ...
    store.Error = function ( options ) {
        ...
        ///  - `error.code` - An integer [error code](#error-codes). See the [error codes](#error-codes) section for
        // more details.
...
(function () {
    ...
    ///  - null
    ///  - object with attribute `oldPurchasedSkus`, a string array with the old subscription to upgrade/downgrade on
    // Android. See: [android
    // developer](https:
    // //developer.android.com/google/play/billing/billing_reference.html#upgrade-getBuyIntentToReplaceSkus) for more
    // info /  - object with attribute `developerPayload`, string representing the developer payload as described in
    // [billing best practices](https: //developer.android.com/google/play/billing/billing_best_practices.html) / / See
    // the ["Purchasing section"](#purchasing) to learn more about / the purchase process. /
...
(function () {
    ...
    // iOS](https:
    // //developer.apple.com/library/ios/releasenotes/General/ValidateAppStoreReceipt/Chapters/ReceiptFields.html#
    // //apple_ref/doc/uid/TP40010573-CH106-SW1). / / Start [here for Android](https:
    // //developer.android.com/google/play/billing/billing_integrate.html#billing-security). / / Another option is to
    // use [Fovea's reeceipt validation service](http: //reeceipt.fovea.cc/) that implements all the best practices to
...
/// # Random Tips
///
/// - Sometimes during development, the queue of pending transactions fills up on your devices. Before doing anything
// else you can set `store.autoFinishTransactions` to `true` to clean up the queue. Beware: **this is not meant for
// production**. / / # internal APIs / USE AT YOUR OWN RISKS
...
(function () {
     ...
     * The restored transactions are passed to the onRestored callback, so make sure you define a handler for that
     * first.
     *
     */
...
(function () {
    ...
    function storekitFinish ( product ) {
        if ( product.type === store.CONSUMABLE || product.type === store.NON_RENEWING_SUBSCRIPTION ) {
            ...
            if ( transactionId ) {
                ...
                // so this prevented the approved event from firing when re-purchasing a product for which finish
                // failed.
    ...
    //
    // This initiates a chain reaction including [`storekitReady()`](#storekitReady) and
    // [`storekitLoaded()`](#storekitLoaded) ! that will make sure products are loaded from server, set as `VALID` or
    // `INVALID`, and eventually restored ! to their proper `OWNED` status. ! ! It also registers the `storekit`
    // callbacks to get notified of events from the StoreKit API: ! !  - [`storekitPurchasing()`](#storekitPurchasing)
    // !  - [`storekitPurchased()`](#storekitPurchased) !  - [`storekitError()`](#storekitError) !
    var initialized  = false;
    ...
    function storekitError ( errorCode, errorText, options ) {
        ...
        // - set the product back to the VALID state
        // This makes it possible to know which product raised an error (previously, errors only fired on the global
        // error listener, which obscures product id). It also seems more consistent with the documented API. See
        // https: //github.com/j3k0/cordova-plugin-purchase/blob/master/doc/api.md#events and
        // https: //github.com/j3k0/cordova-plugin-purchase/blob/master/doc/api.md#notes
        p = store.get ( options.productId );
        if ( p ) {
            p.trigger ( "error", [ new store.Error ( {
                code    : errorCode,
